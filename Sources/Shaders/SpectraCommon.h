...
inline float3 spectra_blend(float3 base, float3 s, float modeF) {
    ...
    switch (mode) {
        ...
        case 14: return min(float3(1.0), base + s);                // linear dodge (add)
        case 15: return max(float3(0.0), base + s - 1.0);          // linear burn
        default: return s;
    }
}
...
// Combine an effect's processed output with its input using explicit universal
// values (blend mode + amount, then strength * opacity). The fused color-pass
// interpreter calls this directly with per-op universals.
inline float3 spectra_compositeExplicit(float3 base, float3 processed,
...
inline float3 spectra_composite(float3 base, float3 processed, constant SpectraUniforms &u) {
    return spectra_compositeExplicit(base, processed, u.strength, u.opacity, u.blendAmount, u.blendMode);
}
...
// Like spectra_composite but WITHOUT the strength cross-fade. Geometric effects
// (warps, CRT curvature) must scale their own displacement/intensity by strength:
// cross-fading a *displaced* image back over the original ghosts it into a double
// image. These effects fold strength into their geometry and composite at full
// strength here, so opacity and blend still apply but there is no position ghost.
inline float3 spectra_compositeFull(float3 base, float3 processed, constant SpectraUniforms &u) {
    float3 blended = spectra_blend(base, processed, u.blendMode);
    float3 mixed = mix(processed, blended, clamp(u.blendAmount, 0.0, 1.0));
    return mix(base, mixed, clamp(u.opacity, 0.0, 1.0));
}

inline float4 spectra_compositeFullRGBA(float3 base, float3 processed, constant SpectraUniforms &u) {
    return float4(spectra_compositeFull(base, processed, u), 1.0);
}
